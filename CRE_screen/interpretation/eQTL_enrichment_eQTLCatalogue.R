...
#mean ORs per tissue
df_ext %>% 
  group_by(simplified_tissue_label) %>%
  summarise(mean_log_OR = mean(log_OR_prioritised_significant, na.rm = T),
            median_log_OR = median(log_OR_prioritised_significant, na.rm = T)) %>%
  arrange(mean_log_OR)

#strongest OR
df_ext %>% 
  arrange(-log_OR_prioritised_significant) %>%
  head(n=1)

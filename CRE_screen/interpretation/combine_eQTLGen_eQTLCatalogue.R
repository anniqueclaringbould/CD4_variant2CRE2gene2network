...
eqtl_interaction_types <- res_combined2 %>%
  filter(is_target == TRUE) %>% #keep only prioritised hits
  filter(significant == TRUE) %>% #keep only significant hits
...
#number of significant and prioritized CRE-gene links
eqtl_interaction_types %>%
  group_by(eQTL) %>%
  summarise(sum = sum(n)) 
234/(234+227) #percentage

#number of significant and prioritized CRE-gene links by interaction type
eqtl_interaction_types %>%
  filter(eQTL == "eQTL") %>%
